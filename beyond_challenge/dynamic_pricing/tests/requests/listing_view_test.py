...
def test_list_with_pagination_successfully(client, list_of_listings, persisted_market):
    ...
    paginated_response = client.get(
        f"/listings?limit={limit}&offset={offset}", content_type="application/json"
    )
    ...
    assert listing.get("market") == persisted_market.label
    assert listing.get("base_price") == str(
        Decimal("100.00") + Decimal(f"{offset + limit - 1}")
    )
    assert listing.get("currency") == "USD"
...
def test_post_fail_due_to_invalid_currency(client):
    ...
    assert response.json() == {
        "errors": {"currency": ["Value 'BRL' is not a valid choice."]}
    }
